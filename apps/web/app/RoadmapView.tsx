      ...
      <div className="flex items-start justify-between gap-3 border-b border-border pb-4">
        <div>
          <h3 className="flex items-center gap-2 text-lg font-semibold">
            <Flag className="h-5 w-5 text-primary" /> Implementation Roadmap
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">{roadmap.summary}</p>
          <div className="mt-2 flex items-center gap-2 text-sm">
            <CalendarClock className="h-4 w-4 text-muted-foreground" />
            <span className="font-medium">Total estimate:</span>
            <span className="text-muted-foreground">{roadmap.totalEstimate}</span>
          </div>
        </div>
        <DownloadButton
...
function PhaseCard({ phase, index }: { phase: RoadmapPhase; index: number }) {
  return (
    <li className="relative rounded-lg border border-border bg-card p-4">
      ...
      {phase.goal && (
        <p className="mt-1 pl-8 text-sm text-muted-foreground">{phase.goal}</p>
      )}
