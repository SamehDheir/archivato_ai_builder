...
| **Business Analysis** | The discovery pass that runs *before* requirements: the problem and who has it, user segments, competitive landscape, USP, a market read, and whether the MVP is the right cut. Outside-knowledge claims (competitors, market) carry a confidence and a research checklist — the tool has no web access and never presents recollection as fact: a named law, regulator or initiative the client didn't state is generalized to its category, not printed as if researched. Re-running on the same interview keeps the same facts (only the framing re-varies); a `refreshFacts` re-run changes them and logs what moved. Owner-only; never shown to your client |
...
| **Database Design** | Entities, columns, keys, relationships + ER diagram (Mermaid, exportable to Draw.io/SVG/PNG/PDF). Multi-tenant scoping is reconciled against your confirmed business rules: a record the rules describe as **shared across branches** (a patient, customer or member seen at more than one location) never gets a mandatory tenant ownership key, while the transactions that reference it stay scoped to where they happened |
| **API Design** | REST endpoints per module with schemas, guaranteed to cover every database entity (or say why not) + interactive API docs and a working mock server. Every documented field type is reconciled against the database schema, so a `uuid` primary key is documented as `uuid` in path params, request bodies and responses alike — never as an `integer` the API layer guessed |
...
| **Project Economics** | Deterministic monthly hosting bill across 11 providers (100 / 1k / 10k users) — headlined by the host your **System Design actually chose**, with any compliance/residency trade-off flagged before a cheaper alternative is suggested — plus a person-week **effort estimate**, third-party **service subscriptions**, an owner-only **budget reality check**, and an owner-only suggested price from your weekly rate |
